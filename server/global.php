...
function fetchUser($guid) {
    $user = Database::doesExist("mine", "guid", $guid, true);
    return $user;
}

function makeUser($guid) {
    $db = new Database;
    $statement = $db->con()->prepare("INSERT INTO `mine` (`guid`, `ip`) VALUES (?, ?)");
    $statement->execute(array($guid, $_SERVER['REMOTE_ADDR']));
}

function saveUser($post, $guid) {
    $db = new Database;
    $str = "UPDATE `mine` SET `map` = ?, `direction` = ?, `x` = ?, `y` = ?, `steps` = ? WHERE `guid` = ?";
